...
int low(string& s, string& t, vi &sa) {
  int l = 0, r = SZ(s) - 1;
  while (l < r) {
    int m = (l + r) / 2;
    if (s.compare(sa[m], SZ(t), t) < 0) l = m + 1;
    else r = m;
  }
  return l;
}

int upp(string& s, string& t, vi &sa) {
  int l = 0, r = SZ(s) - 1;
  while (l < r) {
    int m = (l + r) / 2;
    if (s.compare(sa[m], SZ(t), t) > 0) r = m;
    else l = m + 1;
  }
  if (s.compare(sa[l], SZ(t), t) == 0) l++;
  return l;
}
...
signed main() {
  ...
  while (q--) {
    string t;
    cin >> t;
    cout << upp(s, t, sa) - low(s, t, sa) << ENDL;
  }
  return 0;
}